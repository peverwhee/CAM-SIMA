...
def write_phys_check_subroutine(outfile, host_dict, host_vars, host_imports,
    ...
    outfile.write("call check_field(file, input_var_names(:,const_input_idx), 'lev', timestep, field_data_ptr(:,:,constituent_idx), ccpp_required_data(req_idx), min_difference, min_relative_value, is_first, diff_found)", 6)
    ...
    outfile.write("call check_field(file, [ccpp_required_data(req_idx)], 'lev', timestep, field_data_ptr(:,:,constituent_idx), ccpp_required_data(req_idx), min_difference, min_relative_value, is_first, diff_found)", 6)
    outfile.write("if (diff_found) then", 6)
    outfile.write("overall_diff_found = .true.", 7)
    outfile.write("end if", 6)
