...
###############################################################################
    ...
    def write_initial_value(self, outfile, indent, init_var, ddt_str, physconst_vars,
        ...
        if tstep_init:
            ...
            outfile.write(f"if ({init_var} .and. (.not. is_initialized('{self.standard_name}', error_on_not_found=.false.) .or. is_parameter('{self.standard_name}', error_on_not_found=.false.))) then", indent)
